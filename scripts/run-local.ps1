...
$worker = Start-Process powershell -PassThru -ArgumentList "-NoExit", "-Command", "go run ./cmd/heimdall-worker"
Start-Sleep -Seconds 2  # give the worker a head start so its internal API is up before the controller's first health check

$controller = Start-Process powershell -PassThru -ArgumentList "-NoExit", "-Command", "go run ./cmd/heimdall-controller"
